...
% if (zeta<0 || zeta>1)
%     error('zeta must be between 0 and 1 inclusive.');
% end;
...
blat=nan(M,1);
for i=1:M
%     eval(['jb=blocks.' char(blocknames(i)) ';']);
...
%         G2 = [G2;
%             zeros(1,3*(j-1)) 0                      -r0*sind(theta0)*delphi -r0*deltheta zeros(1,3*(M-j));
%             zeros(1,3*(j-1)) r0*sind(theta0)*delphi  r0*deltheta             0           zeros(1,3*(M-j))];
