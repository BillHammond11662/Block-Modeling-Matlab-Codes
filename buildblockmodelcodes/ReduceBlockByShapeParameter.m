function [blocks,nodes,faults]=ReduceBlockByShapeParameter(blocks,nodes,faults,bnds,Rmax,Amin,Amax,optverb)
% [blocks,nodes,faults]=ReduceBlockByShapeParameter(blocks,nodes,faults,bnds,Rmax,Amin,Amax,optverb)
% 
% Remove blocks in model with shape parameter greater than Rmax
%
% Routine tried to combine high space parametr blocks with adjacent blocks touching longest edge of 
% the high R block that is not a fault or model boundary. 

[~,R,~,~,~,~,~,~] = blockgeoms(blocks,nodes);

% remove each block by taking its longest side that is not a fault segment
bnames = fieldnames(blocks);
[Rsrt,isrt]=sort(R);
bnames=bnames(isrt);
bnamesout = flipud(bnames(Rsrt>Rmax));

if isempty(bnamesout)
    disp(['All blocks have shape parameter less than ' sprintf('%.2f',Rmax) '. Nothing to do.']);
    return;
else
    disp(['Working on ' sprintf('%.2f',length(bnamesout)) ' blocks that have high R.']);
end

for b=bnamesout'
    
    if optverb
       disp(['Trying to remove high shape parameter block #: ' char(b)]);
    end
    ...
    if isfield(blocks,char(b))
        ...
        if isempty(seglist)
            ...
            % if fault does not exist there, add it
            ...
            for k=1:length(seglist)
                sn=seglist(k);
                if optverb
                   disp(['   Testing exceedence on  seg #: ' num2str(sn)]);
                end
                ...
                [blocks2,~,~,~]=RemoveBlock(blocks,nodes,faults,segnds);
                
%                 faults(frm,:)=[];  % remove provisional fault
                ...
                for q=1:length(blockschanged)
                   ...
                   % nn2=eval(['blocks2.' char(blockschanged(q))]);
                    nn2= blocks2.(char(blockschanged(q)));
            ...
            if min(exceed)<=exceed0
                ...
                if optverb
                   disp(['Warning: Not combining block ' char(b) ' RemoveBlock returned empty.']);
                end
